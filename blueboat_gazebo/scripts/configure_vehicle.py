...
"""
Generate every artifact of a BlueBoat configuration from one config file.
...
vehicle be reconfigured without rebuilding, also from a binary install:
    ...
    ros2 run blueboat_gazebo configure_vehicle.py --config my_vehicle.yaml --out-dir ~/my_blueboat
    ros2 launch blueboat_gazebo sim.launch.xml config_file:=my_vehicle.yaml   # does this for you
...
topic_namespace (blueboat by default) names the instance, as before.
"""
...
<model>
  ...
  <description>BlueBoat, generated by configure_vehicle.py from a vehicle config.</description>
</model>
"""
...
def instance_name(config_text):
    ...
    return (yaml.safe_load(config_text) or {}).get('topic_namespace', 'blueboat')
...
def main(argv=None):
    ...
    if args.cache:
        ...
        print(f'wrote vehicle.yaml, blueboat.urdf, blueboat.gazebo.urdf, model.sdf, model.config, '
