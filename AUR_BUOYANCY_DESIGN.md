...
`fluid_density` is a known wrinkle. Turning a declared net mass into displaced volume requires a density, and the density is also a user parameter of the world's Buoyancy plugin, in a different file and possibly a different project. Nothing can force the two to agree, so the rule is documented instead: both default to 1025 for seawater, and if you change one, change the other. Declaring it beside the trim keeps the assumption visible, and a test checks the playground world shipped in this repo.
