...
The first four words are used interchangeably in most conversations about textures, which causes confusion The glTF hierarchy runs image, then texture, then the material property that points at one. % CLAUDE: At one?  You mean the material property points at a texture?   
The last three are here because the Prims and Mats columns turn on them, and because the review has an open question about what "one mesh" means (section 10, decision 15).
...
- **texture** — glTF's `textures` array: one image paired with a sampler, which says how the image is filtered and wrapped when it is read. A texture is a way of reading an image, so two textures can share one image and read it differently. So `texture` = one image + one sampler; a texture is "a way of reading an image."
...
- **material** — the PBR parameter block a primitive points at: the factors, the alpha mode, and the five map properties above.
