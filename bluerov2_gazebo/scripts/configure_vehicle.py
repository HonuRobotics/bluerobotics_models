...
"""
Generate every artifact of a BlueROV2 configuration from one config file.
...
vehicle be reconfigured without rebuilding, also from a binary install:
    ...
    ros2 run bluerov2_gazebo configure_vehicle.py --config my_vehicle.yaml --out-dir ~/my_bluerov2
    ros2 launch bluerov2_gazebo sim.launch.xml config_file:=my_vehicle.yaml   # does this for you

With --cache the directory is one under $ROS_HOME (default ~/.ros) named
after the config's content, so the same config always lands in the same
...
topic_namespace (bluerov2 by default) names the instance, as before.
"""
...
<model>
  ...
  <description>BlueROV2, generated by configure_vehicle.py from a vehicle config.</description>
</model>
"""
...
def instance_name(config_text):
    ...
    return (yaml.safe_load(config_text) or {}).get('topic_namespace', 'bluerov2')
...
def configure(config, out_dir, name=None):
    ...
    desc = pathlib.Path(get_package_share_directory('bluerov2_description'))
    gz = pathlib.Path(get_package_share_directory('bluerov2_gazebo'))
    gz_lib = pathlib.Path(get_package_prefix('bluerov2_gazebo')) / 'lib' / 'bluerov2_gazebo'
    ...
    vehicle_xacro = str(desc / 'urdf' / 'bluerov2.urdf.xacro')
    ...
    run([sys.executable, str(gz_lib / 'generate_model.py'), str(config),
         str(urdf), str(gz / 'model.sdf.xacro'), str(out_dir / 'model.sdf'),
         f'file://{gz_urdf}'],
        'model generation')
...
def cache_dir(config, name=None):
    ...
    return ros_home / 'bluerov2_gazebo' / digest
...
def main(argv=None):
    ...
    group.add_argument('--cache', action='store_true',
                       help='write into $ROS_HOME/bluerov2_gazebo/<config hash>, print its path')
    ...
    if args.cache:
        ...
        print(f'wrote vehicle.yaml, bluerov2.urdf, bluerov2.gazebo.urdf, model.sdf, model.config, '
